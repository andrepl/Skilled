package com.norcode.bukkit.skilled.command;
...
public class BaseCommand implements TabExecutor {
	...
	public BaseCommand(Skilled plugin, String name, String[] aliases, String requiredPermission, String[] help) {
		this.plugin = plugin;
		this.name = name;
	...
	public final boolean onCommand(CommandSender commandSender, org.bukkit.command.Command command, String label, String[] args) {
		try {
			...
			commandSender.sendMessage(error.getMessage());
		}
		return true;
	}
	...
	protected void showHelp(CommandSender sender, String label, LinkedList<String> args) {
		...
		if (subcommands.size() > 0) {
			sender.sendMessage("Sub-Commands: " + StringUtils.join(filterByPermission(sender, subcommands).keySet(), ", "));
		}
	}
